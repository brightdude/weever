...
- (id)init
{
    ...
    if (self) {
        ...
        self.numberOfSections = [PMLocationViewController isLocationUpdatingEnabled] ? 3 : 1;
    }
    return self;
}
...
-(void)loadView
{
    ...
    NSString* headerText;
    if ([PMLocationViewController hasBackgroundModeEnabled]) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section) {
        case 0:
            return [PMLocationViewController hasBackgroundModeEnabled] ? 2 : 1;
        case 1:
            return 2;
        case 2:{
    ...
    switch (indexPath.section) {
        case 0:
        {
            ...
            switch (indexPath.row) {
                case 0:
                    if ([PMLocationViewController hasBackgroundModeEnabled]) {
                        toggleSwitch.on = [PMLocationViewController isBackgroundUpdatingEnabled];
                ...
                default:
                    ...
                    toggleSwitch.on = [PMLocationViewController isLocationUpdatingEnabled];
        ...
        case 1:
        {
            ...
            cell.userInteractionEnabled = [PMLocationViewController isLocationUpdatingEnabled];
...
-(void) updateGeofencedLocations
{
    ...
    NSArray *regionArray = [[[PMLocationManager sharedLocationManager] monitoredRegions] allObjects];
    for (int i = 0; i < [regionArray count]; i++) {
        [[PMLocationManager sharedLocationManager] stopMonitoringForRegion:[regionArray objectAtIndex:i]];
    }
    
    if ([PMLocationViewController isLocationUpdatingEnabled]) {
        
        NSMutableArray *geofences = [NSMutableArray array];
        NSArray* locations = [PMLocationViewController locationsForUpdate];
        ...
        if (geofences.count > 0) {
            for(CLRegion *geofence in geofences) {
                [[PMLocationManager sharedLocationManager] startMonitoringForRegion:geofence];
            }
        }
...
{
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    return [userDefaults boolForKey:kBackgroundUpdates] && [PMLocationViewController hasBackgroundModeEnabled];
}
...
{
    [PMLocationViewController setDefaultDefaultsValue:isBackgroundingEnabled forKey:kBackgroundUpdates];
}

+ (void) setDefaultLocationUpdateValue:(BOOL) areLocationUpdatesEnabled
{
    [PMLocationViewController setDefaultDefaultsValue:areLocationUpdatesEnabled forKey:kLocationUpdates];
}
